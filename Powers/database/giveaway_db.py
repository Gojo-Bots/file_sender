...
class GIVEAWAY(MongoDB):
    ...
    def save_give(
        ...
        voting:int = 0, #Voting started? 0 for no 1 for yes
    ...
    def update_ga_id(self, u_id, ga_id):
        with INSERTION_LOCK:
            curr = self.find_one({"user_id":u_id})
            if curr:
                self.update({"user_id":u_id},{"giveaway_id":ga_id})
    ...
    def voting_start(self, chat_id,update = False,val=0):
        with INSERTION_LOCK:
            curr = self.find_one({"chat_id":chat_id})
            if curr:
                if update:
                    self.update({"chat_id":chat_id},{"voting":val})
                    return
                else:
                    if curr["voting"]:
                        return True
                    elif not curr["voting"]:
                        return False
